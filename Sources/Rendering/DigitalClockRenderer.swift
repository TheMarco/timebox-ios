...
/// scrolling "Artist — Title" ticker (7px pixel font) below it. The time uses a small
...
enum DigitalClockRenderer {
    ...
    private static let tickerTopY = 9 // 5px Pixelify ticker occupies rows 9...13
    ...
    static func frame(for date: Date, ticker: String = "", scroll: Int = 0,
        ...
        // --- Ticker below: single left-to-right pass. Starts left-aligned (scroll 0)
        // and slides off the left as `scroll` grows; the engine ends the pass once it's
        // fully gone, then crossfades to the next view. ---
